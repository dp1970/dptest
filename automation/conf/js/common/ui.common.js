	...
	ajax:function(options){
		...
		var defaults = {
		   ...
		   beforeSend:function(){ //ajax发送之前做的操作
			   if(opts.btn != null){
				   opts.btn.attr('disabled','disabled'); //不可点
			   }
			   if(opts.icon != null){
				   opts.icon.addClass("fa fa-spinner fa-spin") //等待
			   }
		   },
		   ...
		   success: function(resp, textStatus, jqXHR){   //接受到返回值
			   switch (resp.status) {
				   case 0: // if resp.status==0
		            ui.msg.success( resp.msg );  //用来展示结果
...
ui = window.ui ||{
	...
	reload:function(){
		...
		},1000); //延时1000毫秒，才执行function
	},
	confirm:function(msg,myCallBack,appendHtml){   //确认框
